...
describe("Basic functionality", _ => {
    ...
    it("should print error message and stack correctly", done => {
        try {
            ...
            assert.equal(
                lines[0].includes(
                    "ReferenceError: undefinedVar1 is not defined"
                ),
                true
            );
    ...
    it("should print objects correctly", done => {
        const data = parseLine(
            pinoMock({ keyOne: "1", keyTwo: "2" }, true),
            argvMock
        );
        const lines = data.split("\n");
...
describe("Command line arguments", _ => {
    it("should accept a --date option", done => {
        ...
        const withDateData = parseLine(pinoMock({ time: date.getTime() }), {
            ...argvMock,
            date: true
        });

        const withoutDateData = parseLine(pinoMock({ time: date.getTime() }), {
            ...argvMock,
            date: false
        });
    ...
    it("should accept a --pid option", done => {
        const mock = pinoMock();
        const withPid = parseLine(mock, { ...argvMock, pid: true });
        const withoutPid = parseLine(mock, { ...argvMock, pid: false });
        const pid = process.pid;

        assert.equal(withoutPid.includes(`[${pid}]`), false);
        assert.equal(withPid.includes(`[${pid}]`), true);
    ...
    it("should accept a --hostname option", done => {
        ...
        const withoutHostname = parseLine(mock, {
            ...argvMock,
            hostname: false
        });
    ...
    it("should accept a --stacktrace option", done => {
        try {
            ...
            const withStacktrace = parseLine(mock, {
                ...argvMock,
                stacktrace: true
            });
            const withoutStacktrace = parseLine(mock, {
                ...argvMock,
                stacktrace: false
            });
    ...
    it("should accept a --colorize option", done => {
        ...
        const withoutColorize = parseLine(mock, {
            ...argvMock,
            colorize: false
        });
    ...
    it("should accept a --messageKey option", done => {
        ...
        const withCustomKey = parseLine(mock, {
            ...argvMock,
            messageKey: "realMessage"
        });
