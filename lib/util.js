...
const parseLine = (line, argv = {}) => {
    try {
        ...
        let rightPointer =
            timestamp || extras ? ` ${figures.pointerSmall} ` : "";
...
const formatExtras = ({
    data,
    options: { colorize: shouldColorize, pid: showPid, hostname: showHostname }
}) => {
...
const formatTimestamp = ({
    data,
    options: { time: showTime, date: showDate, timestampKey, colorize }
}) => {
    ...
    const formattedDate = format(
        date,
        `[${dateFragment}${delimiter}${timeFragment}]`
    );
...
const formatLevel = ({ data: { level }, options: { colorize } }) => {
    ...
    const formattedString = figures(
        levelColor(levelFigure + " " + levelString)
    );
...
const formatMessage = ({
    ...
    if (message === undefined) {
        ...
        const printableObject = Object.keys(data)
